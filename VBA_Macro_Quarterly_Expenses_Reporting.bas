...
' prompt user to know which quarter is reported
Function GetQuarterFromUser() As Integer
    Dim q As Variant
    
    q = Application.InputBox( _
            Prompt:="Enter quarter number (1–4):", _
            Title:="Select Quarter", _
            Type:=1)
    
    If q < 1 Or q > 4 Then
        MsgBox "Invalid quarter. Please enter 1, 2, 3, or 4.", vbExclamation
        GetQuarterFromUser = 0
        Exit Function
    End If
    
    GetQuarterFromUser = q
End Function
