...
- For each data page, the _Weekday Tooltip_ page contains _Revenue for Top 3 Categories_ bar graph is an enhenced tooltip which shows the top 3 revenue generating product categories depending on how the data is filtered by the selection of data elements on one of the other pages. This page is not visible to report readers.
...
- **Unpivot** page contains unpivoted cost data. _Cost by Store ID and Cost Type_ is a stacked column chart, columns are organized by store_id, and the portion of cost is color coded by cost type within each column. There is also a bar chart for sum of revenue per state. This page also contains 2 slicers. _Filter by Store ID_ filters by state and within each state by city; _Price Slider_ is a slider for filtering by interval of price range of products sold, to be included in the revenue aggregation.
- **Details** page contains date based KPIs with formatted dates. _Order Date and Revenue_ tables contain order dates with the sum of orders and the sum of revenue for that date. _Product Package Volume_ table contains the name of the product - the width, length and depth dimensions of its packaging as well as the calculated column for Volume. _Revenue by Weekday_ is a bar graph which displays revenue by weekdays. The page also displays the _Total Revenue_ and the _Total Sales_.
-  The **Maps** page displays information related to teh store dimension of the data. _Revenue by Year, Quarter and Month_ is a line chart with revenue by year, quarter and month, with ML-based forecasting - for the next 10 quarters, taking into account weekly seasonality, at a 95% confidence level. _Revenue per State & City by Latitude & Longitude_ is  a map-based chart which shows amount of revenue by state and city using latitude and longitude to map locations. This page includes a _city_ filter for cities which dynamically flters the graphs when a selectin is made.
-  The **Overview** page which incorporates several charts to visualize revenue. _Revenue by State_ is a pie chart showing by state partitioning of the total enterprise revenue (throughut 2017-2019) shwoing data labels with the actual dollar amounts. _Revenue & Sales by Year_ is a line chart showing revenue (left y-axis) and sales (right y-axis) throughut 2017-2019, aside from built in Power BI drillthrough feature the chart has an added adjustable x-axis feature for expanding into specific segments of the timeline. _Revenue by Category & State_ is a chart containing multiple bar charts - one per state, showing revenue generated by categories. _Revenue by Category_ is a treemap chart partitioned by categories and sub-categories within those categories.
-  The **DAX - calculations** displays data from the **Measures Table** demomnstrating the use of aggregation functions in Power BI. _Base Pkg Dimension by Category_ is a bar graph showing average base area for packages by category. There is a _Promo Details_ table that shows aggregated count of promos by promo types. There is also a display of the avergae layout dimensions (_AVG layout dim_) for product packaging, across all categories. 
-  The **DAX - filters** page contains the table _Cost by City_ which details percent cost by state from total enterprise operational costs for fulfilling orders, with a break down of the actual cost for fulfilling orders by city, with city_id and store_id specification columns.  This page contains a calculation of _total revenue_ using the SUMX aggregation function on sales and price columns of the **sales_total_2** table - this value could be used to validate revenue data entries during audit. There is also a calculation for _All Costs_ across all states where the enterprise operates. The _Logistic Category Costs with Volume_ calculates cost for products in terms of sales and product volume - this would be related to the packaging and delvery costs. The _Logistic Category Costs_ calculates costs of product delivery by taking into account certain tarriffs or additional processing fees specific to products belonging to the Beauty & Hygiene and Baby Care categories. There is also a filter slicer _state_ which affects the values displayed and the table.
-  The **Change in Cost from Previous** page contains **Difference in Cost 2017-2019** table with the _Year_ column - drilled down by quarters and within quarters by months and within months by individual days. The next column is the _Cost of Order_ column, which details order cost for the specific period selected in the _Year_ column. The _Cost Previous_ shows the cost for the same quarter, month or day but one year ago. The _Change in Cost_ column shows the percent difference in cost between the previous year (_Cost Previous_) and current year (_Cost of Order_) - if there is an increase in cost in the current year then the value is positive, if there is a decrease then it is a negative percentage value.
-  The **YTD Cost & Revenue** contains the _Cost of Order, YTD Cost and YTD Revenue by order_date_ line graph which shows the cost of order, the YTD cost and the YTD revenue. There is also a display of the _All Costs_ value and the _YTD Revenue till August 2018_ which come from the **Measures Table**. There is a table showing the percentage of the revenue, which comes from Beauty & Hygiene, by city (_% Revenue from Beauty & Hygiene by City_). There is a table which shows the percentage of sales that happen on Mondays by category (_Monday Sales by Category_). The last table on this page shows the revenue generated by each category as the actual monetary amount and as a percentage share of the total revenue generated across all categories (_Revenue by Category_). The page also contains a slicer to filter data by _state_.
